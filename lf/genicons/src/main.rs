// SOURCED FROM https://github.com/eza-community/eza/blob/main/src/output/icons.rs
...
fn main() -> io::Result<()> {
	...
	fn write_icons(
		...
		iter: impl IntoIterator<Item = (impl Display, impl Display)>,
	) -> io::Result<()> {
		iter.into_iter()
			.try_for_each(|(spec, icon)| writeln!(stdout, "{spec}\t{icon}"))
	}

	writeln!(stdout, "# File types")?;
	write_icons(
		&mut stdout,
		&phf_map! {
			"ln" => SYMLINK,
			"or" => SYMLINK,
			"tw" => Icons::FILE_OUTLINE,
			"ow" => Icons::FILE_OUTLINE,
			"st" => Icons::FILE_OUTLINE,
			"di" => Icons::FOLDER,
			"pi" => Icons::FILE_OUTLINE,
			"so" => Icons::FILE_OUTLINE,
			"bd" => Icons::FILE_OUTLINE,
			"cd" => Icons::FILE_OUTLINE,
			"su" => Icons::FILE_OUTLINE,
			"sg" => Icons::FILE_OUTLINE,
			"ex" => Icons::FILE_OUTLINE,
			"fi" => Icons::FILE_OUTLINE,
		},
	)?;
	writeln!(stdout)?;
	writeln!(stdout, "# Extension icons")?;
	write_icons(
		&mut stdout,
		EXTENSION_ICONS
			.into_iter()
	...
	writeln!(stdout)?;
	writeln!(stdout, "# Directory icons")?;
	write_icons(
		&mut stdout,
		DIRECTORY_ICONS
			.into_iter()
			.map(|(dirname, icon)| (*dirname, *icon)),
	)?;
	writeln!(stdout, "# Filename icons")?;
	write_icons(
		&mut stdout,
		FILENAME_ICONS
			.into_iter()
			.map(|(filename, icon)| (*filename, *icon)),
	)?;
